...
def findTimeForSpeed(v, vPts, tPts, x):
    ...
    for time in tSols:
        if sympy.im(time) <= 10**(-15):
            time = sympy.re(time)
            if tPts[i] - 10**(-10) <= time <= tPts[i + 1] + 10**(-10):
                return time

def integrate(upper, x, t):
    area = 0
    interval = findInterval(upper, t)

    for i in range(interval):
        area += integ.quad(lambda a: f(a, x, t), t[i], t[i + 1])[0] / 3600
    area += integ.quad(lambda a: f(a, x, t), t[interval], upper)[0] / 3600
...
def main():
    ...
    tf = findTimeForSpeed(90, v, t, x)
    print(f'{round(integrate(tf, x, t), 4)} miles')
